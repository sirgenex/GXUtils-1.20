package br.com.srgenex.utils.armor.listener;

import br.com.srgenex.utils.GXUtils;
import br.com.srgenex.utils.armor.event.ArmorEquipEvent;
import br.com.srgenex.utils.armor.event.ArmorEvent;
import br.com.srgenex.utils.armor.event.ArmorUnequipEvent;
...
public class ArmorListener implements Listener {
            ...
            for (int i = armor.length - 1; i >= 0; i--) {
                if (holding.isSimilar(armor[i])) {
                    ...
                    if (armorEvent.isCancelled()) {
                        ...
                        Bukkit.getScheduler().runTask(GXUtils.getInstance(), () -> {
    ...
    private void onPlayerInteract(PlayerInteractEvent e) {
        ...
        if (!p.getInventory().getItem(equipmentSlot).getType().isAir()) return;

        Bukkit.getScheduler().runTask(GXUtils.getInstance(), () -> {
            if (!p.getInventory().getItem(equipmentSlot).getType().isAir()) {
